...
def register_blotter_callbacks():
    ...
    def poll_for_changes(n_intervals, known_mtime, last_write_time, current_orders):
        """Check if orders.json was modified externally; if so, reload blotter."""
        ...
        if current_mtime == last_write_time:
            return no_update, no_update, current_mtime, no_update

        orders = load_orders()
        blotter_rows = orders_to_display(orders)
        return orders, blotter_rows, current_mtime, True
